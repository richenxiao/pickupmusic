...
不同来源下载的本地音乐，往往缺少对应封面、歌词或完整的专辑信息；普通播放器通常只能把文件播放出来，如想获得完整如同流媒体的体验，用户要付出高昂的时间成本去手动添加附带内容。拾音便是为了解决这个痛点开发的。
...
- **对话式整理**：首页进入 Agent，直接说人话——「把《XX》按官方顺序排一下」「这首歌歌手识别错了」「这首歌的歌词给我找一下」，AI 联网搜证、逐步执行，每步进度实时可见，写库前经你确认。
- **批量改歌名**：说「把这张专辑每首歌名的 - 歌手名 后缀去掉」「繁体转简体」「片假名换成汉字官方名」，AI 逐首分析、列出「旧名 → 新名」改动清单后写回，随时可在曲目菜单改回；也可以点名「把 A 改成 B」单改一首。
- **问它你的曲库**：「库里有没有这首歌」「我最近在听什么」，回答基于真实音乐库与播放次数，不靠 AI 瞎编；事实性问题会先联网核实再答。
- **自带模型，自选供应商**：DeepSeek、OpenRouter、智谱、通义预设一键填入，或自定义任意 OpenAI 兼容服务商；API Key 只存在你手机本地。设置页可查看 Token 用量趋势（输入 / 输出 / 缓存命中 / 成本曲线）。
